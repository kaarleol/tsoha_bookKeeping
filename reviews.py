...
def get_list():
    ...
    if user_id == 0:
        return False
    sql = text("SELECT B.book_id, R.ID, B.title, B.author, B.publication_date, B.genre, R.read_status, R.rating, R.review, U.username FROM User_Books R, book B, Users U WHERE R.book_id=B.book_id AND R.user_id=:user_id AND R.user_id=U.user_id ORDER BY R.rating")
